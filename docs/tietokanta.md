...
Kentät: order_id, group_name, group_order, item_order, nimike, profile_type, profile_data (jsonb), aihio_width_mm, quantity, material, color, thickness_mm, product_group, production_status.
...
- production_status — kokonaistila, lasketaan käytännössä order_item_rows-riveistä. Arvot: WAITING/CUT/BENT/EXTRA/DONE
...
Kentät: order_item_id, row_order, mitta_mm, paat_mm, kok_mm, kpl, huomio, production_status, done_qty.

- production_status — rivikohtainen tuotantovaihe. Arvot: WAITING/CUT/BENT/EXTRA/DONE, default WAITING
- done_qty — montako kappaletta tästä rivistä on tehty. Integer, default 0. Käytetään osittaisen valmistumisen seurantaan (esim. "4/7 leikattu")

Käyttötarkoitus: leikkausoptimointi, tulostus, työkortti, tuotannonohjaus.
...
Limitysvara saumoille. Tulevaisuuden ominaisuus.
Liittyy order_items:iin — CASCADE poistaa rivit kun order_item poistetaan.
...
Asiakkaat. Täytetty Access-siirrolla (4902 asiakasta).
...
- email — sähköposti
- email2 — sähköposti 2
- ovt_number — OVT-numero verkkolaskutusta varten
- notes — muistiinpanot (huom: vanhoilla asiakkailla notes-kentässä voi olla yhteyshenkilötietoja — korjataan tarvittaessa)

RLS-politiikat:
- SELECT: anon saa lukea
...
| orders/tilaukset.html | Tilauslista ja hallinta |
| orders/tuotanto.html | Tuotannonohjaussivu |
...
SELECT oi.nimike, oi.material, oi.color, oi.thickness_mm, oi.aihio_width_mm,
  r.mitta_mm, r.paat_mm, r.kok_mm, r.kpl, r.production_status, r.done_qty
FROM orders o
...
-- Tuotantotilanne per tilaus
SELECT o.id, c.name, o.kohde,
  COUNT(r.id) as riveja,
  SUM(CASE WHEN r.production_status = 'DONE' THEN 1 ELSE 0 END) as valmiita
FROM orders o
LEFT JOIN customers c ON o.customer_id = c.id
JOIN order_items oi ON oi.order_id = o.id
JOIN order_item_rows r ON r.order_item_id = oi.id
WHERE o.status IN ('ORDERED','IN_PRODUCTION')
GROUP BY o.id, c.name, o.kohde;
